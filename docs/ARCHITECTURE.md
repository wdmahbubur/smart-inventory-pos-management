...
No independent quantity editor, accounting cost layers, returns/reversals, due balances, cash drawer, staff roles, AI database tools, voice services or external analytics backend exist. Current inventory reference-cost value is not itself profit or confirmed supplier payment. For the user-requested dashboard/report metric, completed sale lines capture the product reference cost at checkout and net profit is net sales minus those captured costs; operating expenses are outside this release. Historical document and cost snapshots are independent of later product/supplier/store edits. Future stock reversals require a separately designed, audited feature rather than an undocumented direct edit.
