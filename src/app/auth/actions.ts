...
export async function authenticate(mode:string,_previous:AuthState,form:FormData):Promise<AuthState> {
 ...
 const raw=Object.fromEntries(form);const origin=process.env.APP_URL;
 ...
 if(mode==='register'){
  ...
  const onboarding=await client.rpc('create_owner_store',{});if(onboarding.error)return {message:'Your account exists, but store setup did not finish. Log in to retry onboarding.'};
  redirect('/dashboard');
 }
 if(mode==='forgot-password'){
  ...
  if(error && error.status && error.status>=500)return {message:'The email service is temporarily unavailable. Please retry later.'};
