import {Heading,Stat,Card,Table,Pagination,Empty,Notice,ReportTabs,ProductLabel,Status} from '@/components/ui';
import {ValueBars,StockDonut} from '@/components/charts';
import {ReportPeriod,ExportButton} from './report-controls';
import {FiltersBar} from '@/components/filters';
import {CatalogFilter} from './catalog-filter';
import {report,entity} from '@/lib/server/data';
import {parseFilters} from '@/lib/schemas';
import {money} from '@/lib/money';
import {displayDate} from '@/lib/dates';
import type {ReportKind,Category,Report} from '@/lib/domain';
import type {SearchParams} from './catalog-pages';
...
function Breakdown({data,kind}:{data:Report;kind:ReportKind}) {
 ...
 return <Table headers={headers} empty={!data.rows.length?<Empty title="No matching report rows" description="Choose another period or clear filters. Empty totals remain zero."/>:undefined}>
  {data.rows.map(row=><tr key={row.id}>
   ...
   {inventory?<>
    <td>{row.category_name}</td><td>{row.quantity} {row.unit}</td><td>{row.minimum_stock}</td>
    <td>{money(row.reference_cost_paisa)}</td><td><strong>{money(row.stock_value_paisa)}</strong></td><td><Status status={row.stock_status}/></td>
   </>:<>
...
export async function ReportPage({kind,params}:{kind:ReportKind;params:SearchParams}) {
 const filters=parseFilters(params),data=await report(kind,filters);
 const inventory=kind==='inventory',sales=kind==='sales';
 const category=inventory&&filters.category?await entity<Category>('categories',filters.category):null;
 const title=inventory?'Inventory report':sales?'Sales report':'Purchase report';
 const rows=data.chart.map(row=>({id:row.id,name:row.name,value:inventory?row.value_paisa:sales?row.gross_paisa??'0':row.total_paisa??'0'}));
 return <>
  ...
  <div className="report-columns" style={{marginBottom:22}}>
   <Card title={inventory?'Estimated value by category':sales?'Product sales before discounts':'Received spending by product'} description={inventory?'Current category and reference-cost estimate.':'Top 10 products by value in this period; all products are in the paginated table.'} body>
    <ValueBars rows={rows} label={inventory?'Current category value estimates':sales?'Gross product sales':'Actual received product spending'}/>
   </Card>
   <Card title={inventory?'Stock status':sales?'Sales reconciliation':'Received purchase summary'} body>
    {inventory?<>
  ...
  {inventory&&<FiltersBar stock><CatalogFilter initial={category}/></FiltersBar>}
  <Card title={inventory?'Current inventory snapshot':sales?'Product sales breakdown':'Product purchase breakdown'} description={inventory?'Table filters affect the rows and filtered estimate, not the global summary cards.':'Products are grouped by immutable ID; labels use their latest snapshot in this period.'}>
   <Breakdown data={data} kind={kind}/>
   {inventory&&<div className="card-footer between"><span className="muted small">Filtered estimate · all {data.total} matching products</span><strong>{money(data.filtered_value_paisa??'0')}</strong></div>}
   <Pagination data={data} path={`/reports/${kind}`} params={params}/>
  </Card>
  {!inventory&&!sales&&<div style={{marginTop:22}}><Card title="Supplier breakdown"><Table headers={['Supplier','Received purchases','Received value']} empty={!data.suppliers?.length?<Empty title="No received purchases in this period"/>:undefined}>{data.suppliers?.map(s=><tr key={s.id}><td><strong>{s.name}</strong></td><td>{s.count}</td><td>{money(s.total_paisa)}</td></tr>)}</Table></Card></div>}
  <div style={{marginTop:18}}><Notice>{inventory?'This is a current operational stock estimate, not historical accounting valuation. Exports capture a fresh consistent snapshot at download time.':sales?'CSV exports include every matching sale line (up to 100,000 rows). Each order discount appears only on its first stable line, so summing the discount column cannot double count it.':'CSV exports include every matching received line (up to 100,000 rows), its actual cost, invoice date and receipt timestamp.'}</Notice></div>
 </>;
}
