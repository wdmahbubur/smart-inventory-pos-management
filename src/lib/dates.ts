...
function unsafePath(value:string):boolean{return [...value].some(character=>character==='\\'||character.charCodeAt(0)<=32);}
export function safeNext(raw:string|null|undefined,fallback='/dashboard'):string{
 if(!raw||!raw.startsWith('/')||raw.startsWith('//')||unsafePath(raw))return fallback;
 try{const decoded=decodeURIComponent(raw);if(decoded.startsWith('//')||unsafePath(decoded))return fallback;const url=new URL(raw,'https://inventory.invalid');if(url.origin!=='https://inventory.invalid'||!/^\/(dashboard|products|categories|suppliers|purchases|pos|sales|inventory|reports|insights|reset-password)(\/|$)/.test(url.pathname))return fallback;return url.pathname+url.search;}catch{return fallback;}
}
