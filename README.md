...
No returns, refunds, posted cancellations, manual adjustments, credit/dues, tax, accounting valuation, profit calculation, staff/multi-store access, voice input or hardware integration are included. The estimate is current quantity times editable reference cost, not FIFO or COGS. CSV exports reject more than 100,000 source rows rather than silently truncating. Order discounts appear once per sale in the export. Remember-me controls persistent versus session cookies, not a custom password store; browser session-restore behavior can retain session cookies.
