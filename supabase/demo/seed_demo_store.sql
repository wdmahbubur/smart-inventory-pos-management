-- Dedicated public demo tenant for visual/product evaluation only.
-- This seed is intentionally NOT part of supabase/migrations so normal fresh installs stay empty.
-- Applied once to the approved hosted inventory-management-v2 project through the migration API.
-- Demo login password corresponding to the bcrypt below: Demo12345!
do $seed$
declare
  v_user uuid := 'de000000-0000-4000-8000-000000000001';
  v_beverages jsonb;
  v_snacks jsonb;
  v_grocery jsonb;
  v_personal jsonb;
  ...
  v_soap jsonb;
begin
  if exists(select 1 from auth.users where lower(email)=lower('demo@smartinventory.app')) then
    raise exception 'DEMO_USER_ALREADY_EXISTS';
  end if;
  ...
  insert into auth.users(
    ...
    null,v_user,'authenticated','authenticated','demo@smartinventory.app',
    '$2a$12$PvqY1PEa7I1XDHOfka3tKevemN1VUkcirJgsTgs3ePV44ubHpXk0G',
    now(),'','','','','','','','',
    '{"provider":"email","providers":["email"]}'::jsonb,
    '{"full_name":"Mahin Ahmed","store_name":"Green Mart Grocery"}'::jsonb,
  ...
  values (
    v_user::text,v_user,
    jsonb_build_object('sub',v_user::text,'email','demo@smartinventory.app'),
    'email',now(),now(),now(),'demo@smartinventory.app'
  );
  ...
  perform public.write_purchase(
    jsonb_build_object(
      'supplier_id',v_supplier1->>'id','purchase_date','2026-09-17','note','Opening beverage and snack replenishment',
  ...
  perform public.write_purchase(
    jsonb_build_object(
      'supplier_id',v_supplier2->>'id','purchase_date','2026-09-18','note','Weekly grocery delivery',
      'items',jsonb_build_array(
  ...
  perform public.write_purchase(
    jsonb_build_object(
      'supplier_id',v_supplier3->>'id','purchase_date','2026-09-19','note','Personal care restock',
      'items',jsonb_build_array(
  ...
  perform public.write_purchase(
    jsonb_build_object(
      'supplier_id',v_supplier2->>'id','purchase_date','2026-09-20','note','Draft: planned top-up for next delivery',
      'items',jsonb_build_array(
  ...
  perform public.complete_sale(
    jsonb_build_object(
      ...
      'discount_paisa','1000','cash_received_paisa','50000','customer_name','Rahim Uddin','customer_phone','01700-100001'
    ),gen_random_uuid());
  ...
  perform public.complete_sale(
    jsonb_build_object(
      ...
      'discount_paisa','2000','cash_received_paisa','100000','customer_name','Nusrat Jahan','customer_phone','01800-100002'
    ),gen_random_uuid());
  ...
  perform public.complete_sale(
    jsonb_build_object(
      ...
      'discount_paisa','0','cash_received_paisa','50000','customer_name','Walk-in customer'
    ),gen_random_uuid());
  ...
  perform public.complete_sale(
    jsonb_build_object(
      ...
      'discount_paisa','1000','cash_received_paisa','120000','customer_name','Nearby Office','customer_phone','01900-100003'
    ),gen_random_uuid());
  ...
  perform public.complete_sale(
    jsonb_build_object(
      ...
      'discount_paisa','2000','cash_received_paisa','300000','customer_name','Mizan Store','customer_phone','01700-100004'
    ),gen_random_uuid());
  ...
  perform public.complete_sale(
    jsonb_build_object(
      ...
      'discount_paisa','3000','cash_received_paisa','300000','customer_name','Evening walk-in sales'
    ),gen_random_uuid());
end
$seed$;
