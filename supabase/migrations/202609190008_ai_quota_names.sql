-- Avoid PL/pgSQL variable/column ambiguity at the atomic quota boundary.
create or replace function public.begin_insight(p_language text,p_provider text,p_model text,p_prompt_version text,p_force boolean default false,p_limit integer default 10) returns jsonb language plpgsql security definer set search_path='' as $$
declare s public.stores:=private.store_lock(); context jsonb; cached public.ai_insights; active_lease private.ai_leases; hour_bucket timestamptz:=date_trunc('hour',clock_timestamp()); count_used integer; new_lease_id uuid:=gen_random_uuid();
begin
 ...
 select w.request_count into count_used from private.ai_request_windows w where w.store_id=s.id and w.window_start=hour_bucket;
 if coalesce(count_used,0)>=p_limit then perform private.fail('AI_RATE_LIMITED'); end if;
 insert into private.ai_request_windows(store_id,window_start,request_count) values(s.id,hour_bucket,1)
 on conflict(store_id,window_start) do update set request_count=private.ai_request_windows.request_count+1;
 insert into private.ai_leases(store_id,language,lease_id,expires_at,metadata) values(s.id,p_language,new_lease_id,clock_timestamp()+interval '90 seconds',jsonb_build_object('context',context,'provider',p_provider,'model',p_model,'prompt_version',p_prompt_version))
 ...
 delete from private.ai_request_windows w where w.store_id=s.id and w.window_start<clock_timestamp()-interval '2 days';
