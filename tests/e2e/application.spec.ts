...
test('missing AI keeps source facts and keyboard dialog restores focus',async({page})=>{
 await login(page);await page.goto('/insights');await expect(page.getByRole('heading',{name:'Source facts',exact:true})).toBeVisible();await page.getByRole('button',{name:'Generate insight',exact:true}).click();await expect(page.getByText('OPENROUTER_API_KEY is missing',{exact:false})).toBeVisible();await expect(page.getByRole('heading',{name:'Source facts',exact:true})).toBeVisible();
