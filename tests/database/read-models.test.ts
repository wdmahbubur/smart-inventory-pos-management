...
test('reports reconcile gross, discount, net and one exported discount per order',async()=>{
 const c=await base();const a=await product(c,'REPORT-A'),b=await product(c,'REPORT-B','5000');
 ...
 const r=await rpc<{summary:{total_paisa:string;subtotal_paisa:string;discount_paisa:string};source_rows:{order_discount_paisa:string|null}[]}>(c.user,'get_report',['sales',{from:today(),to:today()},true]);
 assert.equal(r.summary.total_paisa,'33000');assert.equal(r.summary.subtotal_paisa,'35000');assert.equal(r.summary.discount_paisa,'2000');assert.deepEqual(r.source_rows.map(x=>x.order_discount_paisa),['2000',null]);
});
